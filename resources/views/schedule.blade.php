...
@section('content')
  ...
  <ul class="nav nav-tabs">
    <li class="nav-item">
      <a class="nav-link active" aria-current="page" href="{{ route('poForList') }}">Forecast & PO Greige Plann</a>
    </li>
    <li class="nav-item">
  ...
  <div class="card shadow-sm">
    ...
    <div class="card-body table-responsive collapse" id="PoGreige">
      <div class="table-container">
        <table id="itemTablePo" class="table table-bordered table-striped fixed-table">
          <thead>
            <tr id="tableHeadPo">
            </tr>
          </thead>
    ...
    <div class="card-body table-responsive collapse" id="Forecast">
      <div class="table-container">
        <table id="itemTableForecast" class="table table-bordered table-striped fixed-table">
          <thead>
...
@push('scripts')
  ...
  <script>
    ...
    const inputQty = $('#txQty');
    const typeSave = $('#typeSave');
    const inputDate1 = $('#tsDate');
    const inputDate2 = $('#txDate');
    const inputRedDate = $('#txRedDate');

    const spanNoItem = $('#calc_no_item');
    const spanQtyInpt = $('#qty');
    const spanQtyH = $('#calc_qtyh');
    const spanQty = $('#calc_qty');
    const spanDateS = $('#start_date');
    const spanDate = $('#calc_date');
    const spanDays = $('#calc_days');
    const spanMachine = $('#calc_machine');
    ...
    $(document).ready(function () {
      ...
      const dateRange = [];
      const headPo = $('#tableHeadPo');
      const headForecast = $('#tableHeadForecast');
          
      headPo.append('<th style="min-width: 150px;">Keterangan</th>');
      headForecast.append('<th style="min-width: 150px;">Keterangan</th>');
      ...
      for (let i = 0; i < 365; i++) {
        ...
        const style = isSunday ? 'background-color: #ffcccc;' : '';
            
        const thHTML = `<th style="white-space: nowrap; ${style}">${formatted}</th>`;
        headPo.append(thHTML);
        headForecast.append(thHTML);
      }
      ...
      $.ajax({
        ...
        // ====================== PO GREIGE ======================
          const dataDB2 = response.dataDB2;
          const groupedPo = {};
          ...
          dataDB2.forEach(row => {
            ...
            // Format tanggal
            const dateObj = new Date(row.rmp_req_to);
            const yyyy = dateObj.getFullYear();
            const mm = String(dateObj.getMonth() + 1).padStart(2, '0');
            const dd = String(dateObj.getDate()).padStart(2, '0');
            const tgl = `${yyyy}-${mm}-${dd}`;

            const qty = row.qty_total;

            if (!groupedPo[itemCode]) {
              groupedPo[itemCode] = {};
            }
            groupedPo[itemCode][tgl] = qty;
          });

          const tbodyPo = $('#tableBodyPo');
          tbodyPo.empty();

          const trPo = $('<tr></tr>');
          trPo.append('<td><b>PO Greige</b></td>');
                  
          dateRange.forEach(dateStr => {
            const dateObj = new Date(dateStr);
            const isSunday = dateObj.getDay() === 0;
            const bgStyle = isSunday ? 'background-color: #ffcccc;' : '';

            const cellData = [];
            for (const item in groupedPo) {
              const qty = groupedPo[item][dateStr];
              if (qty) {
                cellData.push(`<div class="cell-item" data-subcode="${item}" data-qty="${Number(qty).toFixed(2)}" data-date="${dateObj}" style="cursor:pointer;">
                  <b>${item}</b>
                  <br>${Number(qty).toFixed(2)}
                </div>`);
              }
            }
          
            trPo.append(`<td style="vertical-align: top; ${bgStyle}">${cellData.join('')}</td>`);
          });
        ...
        // ====================== FORECAST ======================
          ...
          forecastData.forEach(row => {
            const itemCode = `${row.item_subcode2}-${row.item_subcode3}-${row.item_subcode4}`;

            const year = today.getFullYear();
            const month = row.buy_month - 1;
            ...
            const qty = row.total_qty_kg;

            if (!groupedForecast[itemCode]) {
              groupedForecast[itemCode] = {};
            }
            groupedForecast[itemCode][tgl] = qty;
          });

          const tbodyForecast = $('#tableBodyForecast');
          tbodyForecast.empty();

          const trForecast = $('<tr></tr>');
          trForecast.append('<td><b>Forecast</b></td>');

          dateRange.forEach(date => {
            const cellData = [];
            for (const item in groupedForecast) {
              const qty = groupedForecast[item][date];
              if (qty) {
                cellData.push(`<div class="cell-item" style="cursor:pointer;">
                  <b>${item}</b>
                  <br>${Number(qty).toFixed(2)}
                </div>`);
              }
            }
            trForecast.append(`<td style="vertical-align: top;">${cellData.join('')}</td>`);
          });

          tbodyForecast.append(trForecast);
        // ====================== FORECAST ======================
        },
        error: function () {
...
<style>
  ...
  .fixed-table {
    ...
    min-width: 1500px; /* Ukuran total tabel minimal */
    width: max-content;
  }

  .fixed-table th,
  .fixed-table td {
    min-width: 150px; /* << penting agar konten tidak mepet */
    padding: 8px;
    text-align: left;
    vertical-align: top;
    white-space: nowrap;
